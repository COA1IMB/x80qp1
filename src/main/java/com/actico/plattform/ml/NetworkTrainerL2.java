...
public class NetworkTrainerL2 {
    ...
    void trainNetwork(List<List<String>> data, List<List<String>> data2, String neuralNetworkFilePath) {
        ...
        boolean useRegulationTestSet = true;
        ...
        while (maxEpoche >= epoche) {
            ...
            if (currentBestF1 < precision && recall != 1.0) {
                currentBestF1 =  precision;
    ...
    private MultiLayerConfiguration getNetworkConfig() {
        return new NeuralNetConfiguration.Builder()
                ...
                .list()
                .layer(0, new DenseLayer.Builder().nIn(38).nOut(2000).updater(ADAGRAD)
                        .gradientNormalizationThreshold(0.1).dropOut(0.9)
                        .weightInit(WeightInit.XAVIER).activation(Activation.RELU).build())
                .layer(1, new DenseLayer.Builder().nIn(2000).nOut(2000).updater(ADAGRAD)
                        .gradientNormalizationThreshold(0.1).dropOut(0.9)
                        .weightInit(WeightInit.XAVIER).activation(Activation.RELU).build())
                .layer(2, new DenseLayer.Builder().nIn(2000).nOut(2000).updater(ADAGRAD)
                        .gradientNormalizationThreshold(0.1).dropOut(0.9)
                        .weightInit(WeightInit.XAVIER).activation(Activation.RELU).build())
                .layer(3, new OutputLayer.Builder(LossFunctions.LossFunction.NEGATIVELOGLIKELIHOOD)
                        .nIn(2000).nOut(2).updater(ADAGRAD)
                        .gradientNormalizationThreshold(0.1).dropOut(0.9)
